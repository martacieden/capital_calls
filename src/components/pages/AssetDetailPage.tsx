...
import type { AnyCatalogItem, Person, Trust, BusinessEntity, Asset, Relationship } from '@/data/types'

/* ── Helpers ── */

const REQUIRED_FIELDS = new Set(['Category'])

function getItemFields(item: AnyCatalogItem): { label: string; value: string | number; required?: boolean }[] {
                                        ...
                                        return (
                                            <div key={f.label} className="detail-page__kv-row flex items-center gap-1 h-8">
                                                <span className="shrink-0 w-32 text-sm font-[var(--font-weight-regular)] text-[var(--color-neutral-11)] whitespace-nowrap flex items-center gap-1 leading-5">
                                                    {f.label}
                                                    {REQUIRED_FIELDS.has(f.label) && <span className="text-[var(--color-red-9)] text-xs font-medium">*</span>}
                                                </span>
                                                {isEditing ? (
                                                    <input
