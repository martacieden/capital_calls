/**
 * ConnectionsGraph — Radial "spider web" ReactFlow graph showing
 * non-hierarchy relationships with the current item at center.
 */
...
import { buildConnectionsGraph } from '@/lib/helpers/radial-layout'
...
function ConnectionsGraphContent({ currentItem, relationships, getItemById, onItemClick, onAction, sidebarOpen }: ConnectionsGraphProps) {
    ...
    const { nodes: baseNodes, edges: baseEdges } = useMemo(
        () => buildConnectionsGraph(currentItem, relationships, getItemById),
        [currentItem, relationships, getItemById],
    )

    // Collect all items for the legend
    const allItems = useMemo(
        () => baseNodes.map(n => (n.data as FtvNodeData).item),
        [baseNodes],
    )
    ...
    useEffect(() => {
        ...
        setRfNodes(prev => prev.map(n => {
            const isCenterItem = n.id === currentItem.id
            const isHovered = n.id === hoveredId
            const isHighlighted = anyHover
                ? isHovered || isCenterItem
                : isCenterItem
    ...
    }, [hoveredId, currentItem.id, stableOnItemClick, setRfNodes, setRfEdges])
